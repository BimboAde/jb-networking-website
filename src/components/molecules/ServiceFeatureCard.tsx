...
export const ServiceFeatureCard = ({ dict, titleKey, descriptionKey, features, Icon, badgeText, accent = 'default' }: ServiceFeatureCardProps) => {
  ...
  return (
    <div
      ...
      } p-8 sm:p-10 shadow-lg hover:shadow-2xl hover:-translate-y-1 hover:ring-2 hover:ring-brand-green/20 focus-within:ring-2 focus-within:ring-brand-green/30`}
    >
      {badgeText && (
        <div
          className={`${
      ...
      <div className="flex items-start gap-4 sm:gap-6">
        <div
          ...
          } w-16 h-16 sm:w-20 sm:h-20 rounded-xl flex items-center justify-center flex-shrink-0 shadow-md transition-transform duration-300 group-hover:scale-105`}
        >
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            {features.map((f) => (
              <div
                key={f}
                className={`flex items-center ${isGold ? 'text-yellow-100' : 'text-brand-green'} transition-transform duration-200 group-hover:translate-x-[1px]`}
              >
