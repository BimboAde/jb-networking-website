...
import { FaFileInvoiceDollar, FaChartLine, FaShieldAlt, FaBriefcase, FaHome, FaEllipsisH } from 'react-icons/fa';
          ...
          {items.map((item, idx) => (
            <div key={idx} className={`bg-brand-gray rounded-lg p-6 hover:shadow-lg transition-shadow ${item.highlight ? 'border-2 border-brand-gold' : ''}`}>
              <div className="flex items-center mb-4">
                <div className={`w-10 h-10 ${item.highlight ? 'bg-brand-gold' : 'bg-brand-green'} rounded-lg mr-3 text-white flex items-center justify-center`}>
                  {[FaFileInvoiceDollar, FaChartLine, FaShieldAlt, FaBriefcase, FaHome, FaEllipsisH][idx]?.({ className: 'text-base' })}
                </div>
                <div>
