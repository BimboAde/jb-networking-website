    ...
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16">
          {items.map((k) => (
            <div key={k} className={`bg-brand-gray rounded-xl p-8 text-center ${k === 'days' ? 'border-2 border-brand-gold relative' : ''}`}>
              {k === 'days' && (
              ...
              <div className={`w-16 h-16 ${k==='written'?'bg-brand-navy':'bg-brand-green'} rounded-full mx-auto mb-6`} />
              <h3 className="text-xl font-bold text-brand-green mb-4 font-poppins">{t(`${k}.title`)}</h3>
              <p className="text-gray-600 mb-4">{t(`${k}.text`)}</p>
              <div className="text-brand-green font-semibold">{t(`${k}.footer`)}</div>
            </div>
          ))}
        </div>
        <div className="bg-gradient-to-r from-brand-green to-brand-light-green rounded-2xl p-8 text-white text-center">
          ...
          <button className="bg-white text-brand-green px-8 py-4 rounded-lg font-semibold text-lg hover:bg-gray-50 transition-colors">
            {t('cta.button')}
          </button>
        </div>
      </div>
